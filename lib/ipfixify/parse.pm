...
This module contains functions related to parsing data to export as IPFIX.
...
Allows an IP address and port to send the IPFIXify data somewhere else in
stream mode.

=item * sourceIp

The source IP allows a users to send data from a different interface (by IP)
on the same system.
...
For system metrics mode on windows, this specifies which system to poll
...
sub cfgCheck {
	...
	my (
		$errList, $ini, $columnCount, $collectorCount, $pidfile, $pidDef,
		$filename, $perm, $columns, $columnCheck
	);
	...
	if ($arg{'sourceip'}) {
		if (! &ipfixify::parse::v_ip($arg{'sourceip'})) {
			$errList .= "\n\n* The source IP defined as '$arg{'sourceip'}' is invalid\n";
		} else {
	...
	if (! $arg{'stream'} && ! $arg{'honeynet'}) {
	  ...
	  ### NOW PROCESS OPTIONAL SETTING NOT ALWAYS THERE IN THE CFG ###
		...
		if ($ini->val('options', 'usernamesOnly')) {
			$cfg{'usernamesOnly'} = $ini->val('options', 'usernamesOnly');
		}
		...
		if ($ini->val('options', 'member')) {
			...
			if (@members > 1 && ! $ini->val('options', 'credentials')) {
				print "\n** ERROR: Members are defined, but there are no ".
					"credentials.\n\n  Please define credentials by executing ".
  ...
  ### CARRY ON ###
	...
	if (! $cfg{collector} && ! $arg{'stream'} && ! $arg{'syspoll'} && ! $arg{'honeynet'}) {
		print "\n** Error: No collectors specified in $arg{'config'}\n";
		exit(0);
	}
	...
	if ($perm > 1) {
		print "\n\n** Error: Only one mode can be enabled on the command line.";
		exit(0);
	}
	...
	foreach (@collector) {
		...
		if ($error) {
			$errList .= "\n\n** Error in Collector #$collectorCount Definitions **\n$error";
		}
	}
	...
	foreach my $field (@columns) {
		...
		if ($field =~ m/\",\"/) {
		  ## OLD FORMAT
			...
			$columns .= lc("$line[3]($ele)")."<$line[5]>{$line[6]$a2b}\n";
		} else {
		  ## NEW FORMAT
			$field =~ s/\"//g;
			$columns .= "$field\n";
	...
	foreach(split (/\n/, $columns)) {
		...
		if ($arg{'verbose'} > 1) {
			print "\n+ Checking column definitions line $columnCount\n\n".
...
the flow cache we are processing at the time. Primarily used for validating
data
...
This should be the address that the originator column will use. If there is no
originator configured, then this data is discarded and not included in the
flow.

=item * flowCache

a reference to the existing flowCache for originators to reference cached
host names
...
sub fileLine {
	...
	if ($cfg->{'mode'} eq 'syslog') {
		...
		$arg{'line'}->{'addr'} = &ipfixify::util::findSourceIpAddressInMsg(
			msg			=> $arg{'line'}->{'msg'},
			original	=> $arg{'line'}->{'addr'}
		);

		if ($arg{'line'}->{'msg'} =~ m/spoofaddress/) {
			my ($a, $addr, $b) = split(/spoofaddress/, $arg{'line'}->{'msg'});
		...
		my ($date, $time, $year, $mon, $mday, $hour, $min, $sec, $epoch, $flow);
		my (@hash_key);
		...
		for ( @{ $arg{line} }{@hash_key} ) {
			...
			# Encode::enecode or you may not get the results you expect.
		...
		$arg{line}->{'time_created'} = timegm(int($sec), $min, $hour, $mday, $mon-1, $year-1900);
		...
		if ($cfg->{'delimiter'} eq 'QUOTEDCSV') {
			...
			if ($cfg->{'originator'}) {
				($originator, $count) = &ipfixify::util::determineOriginator(
					data		=> \@line,
					cfg			=> $cfg,
					columns		=> $columns,
				);
			...
			# bug 17373 - send a flow per recipient instead of all in one field
			if ($raw[11] =~ m/;/) {
				my (@recipients);
	...
	foreach my $queued (@work) {
		...
		foreach (split (/\n/, $columns)) {
			...
			# need this to force a value of one for ipfixifyMessageCount
			# if the column exists in the template and that value is empty
			...
			# for mailinizer support (bug 12008)
			# this field isn't matching what the docs say, but the others are
			if ($ele eq '13745.109') {
				$flow[$count] = '0' if ($flow[$count] =~ m/\D/ || ! $flow[$count]);
			}

			# What is probably proper is to split out into different templates
			# and turn mailinizer into a mode for IPv6
			if ($ele eq '13745.100' || $ele eq '13745.104' || $ele eq '13745.131') {
				# ::1 in v4 fields (bug 12008)
				$flow[$count] = '127.0.0.1' if ($flow[$count] eq '::1');
				# IPv6 addresses and missing addresses turn to N/A (bug 17373)
			...
			## bug 12008, this corrects non-integers for integer fields
			## moved to here due to bug 13081
			...
			if (! defined $flow[$count]) {
				print "cache $arg{'cacheid'} field $count [$element->{'dataType'}]".
		...
		if (@flow != $colCount) {
			print "MISMATCH [$arg{'cacheid'}]: ". scalar(@data). " [expected $colCount]\n" ;
			print Dumper \@flow;
...
sub date_time_to_epoch {
	...
	eval {
		my $dt = DateTime::Format::ISO8601->parse_datetime($arg{'date'});
		$epoch = $dt->epoch;
	};
...
This function takes a honeynet flow and parses it up and sends it out as IPFIX.
