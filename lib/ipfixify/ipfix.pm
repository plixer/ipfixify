...
sub getFDIHandle {
	...
	die "FDI args must be a HASH ref" unless ( ref $fdi_args eq 'HASH' );
...
This function forms the IPFIX option template for rfc 3164 information.
...
This function forms the IPFIX option template based on the configuration column
settings for rfc5610 support.
...
sub rfc5610optionTemplate {
	...
	foreach(split (/\n/, $arg{'columns'})) {
		...
		if ($arg{'verbose'} > 1) {
			print "  - option template data flow\n\n".
				...
				"    0_339 : $element->{'dataType'} (".$dataType{lc($element->{'dataType'})}.")\n".
				"    0_340 : ''\n".
				"    0_341 : $element->{'name'}\n".
				"    0_342 : 0\n".
				"    0_343 : 0\n".
				"    0_344 : $element->{'dataTypeSemantics'} (".$semantics{lc($element->{'dataTypeSemantics'})}.")\n".
				"    0_345 : 0\n".
	...
	foreach my $collector (@{$arg{'cfg'}->{'collector'}}) {
		if (! $arg{'flowCache'}->{'fdh'}{"$collector-SELF"}) {
			...
			$arg{'flowCache'}->{'fdh'}{"$collector-SELF"} = &ipfixify::ipfix::setupFdh(
				ip		=> $sendAddr,
				port	=> $sendPort,
			);
		} else {
...
sub sendFlows {
	...
	if ($arg{'cfg'}->{'mode'} eq 'sysmetrics') {
		...
		foreach $file (@queue) {
			...
			foreach $line (@spool) {
				push (@{$arg{'flowCache'}->{$fCache}{'flows'}{'SELF'}},
			...
			unlink "$ENV{'TMPDIR'}/$file" if (-e "$ENV{'TMPDIR'}/$file");
		}
		#print "Spooling (End)\n";
	}
			...
			foreach (keys %copyCache) {
				...
				foreach my $collector (@{$arg{'cfg'}->{'collector'}}) {
					if (! $arg{'flowCache'}->{'fdh'}{"$collector-$_"}) {
						...
						$arg{'flowCache'}->{'fdh'}{"$collector-$_"} = &ipfixify::ipfix::setupFdh(
							ip		=> $sendAddr,
							port	=> $sendPort,
						);
					} else {
						...
						$arg{'flowCache'}->{'fth'}{"$collector-$_"} =
							$arg{'flowCache'}->{'fdh'}{"$collector-$_"}->prepare(
